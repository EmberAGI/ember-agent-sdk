/*
 * Ember API
 *
 * Comprehensive specifications for all Ember API endpoints, incorporating the complete set of DeFi capabilities.
 *
 * The version of the OpenAPI document: 0.2
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct WithdrawLentTokensResponseResult {
    /// Transaction details.
    #[serde(rename = "transactionPlan", skip_serializing_if = "Option::is_none")]
    pub transaction_plan: Option<serde_json::Value>,
    #[serde(rename = "feeBreakdown", skip_serializing_if = "Option::is_none")]
    pub fee_breakdown: Option<Box<models::FeeBreakdown>>,
    #[serde(rename = "withdrawalDetails", skip_serializing_if = "Option::is_none")]
    pub withdrawal_details: Option<Box<models::WithdrawLentTokensResponseResultWithdrawalDetails>>,
}

impl WithdrawLentTokensResponseResult {
    pub fn new() -> WithdrawLentTokensResponseResult {
        WithdrawLentTokensResponseResult {
            transaction_plan: None,
            fee_breakdown: None,
            withdrawal_details: None,
        }
    }
}

